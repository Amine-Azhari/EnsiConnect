    ...
    return Scaffold(
      ...
      body: SafeArea(
        child: _isLoading
            ...
            : _allTutors.isEmpty
                ? const Center(child: Text("Aucun tuteur classé pour le moment."))
      ...
      child: Row(
        ...
        children: [
          if (_topTutorsPodium.length >= 2)
            _buildPodiumItem(_topTutorsPodium[1], 2, const Color(0xFFC0C0C0), isDark, textColor, 70),
          if (_topTutorsPodium.isNotEmpty)
            Hero(
              tag: 'tutor_profile_${_topTutorsPodium[0]["id"]}',
              child: _buildPodiumItem(_topTutorsPodium[0], 1, const Color(0xFFFFD700), isDark, textColor, 110),
            ),
          if (_topTutorsPodium.length >= 3)
            _buildPodiumItem(_topTutorsPodium[2], 3, const Color(0xFFCD7F32), isDark, textColor, 50),
        ],
      ),
    );
  }

  Widget _buildPodiumItem(Map<String, dynamic> tuteur, int rank, Color rankColor, bool isDark, Color textColor, double stepHeight) {
    ...
    return InkWell(
      ...
      child: Column(
        mainAxisSize: MainAxisSize.min,
        children: [
          Icon(Icons.workspace_premium, color: rankColor, size: rank == 1 ? 36 : 28),
            ...
            children: [
              Container(
                ...
                child: CircleAvatar(
                  ...
                  backgroundImage: tuteur["imageUrl"] != null ? NetworkImage(tuteur["imageUrl"]) : null,
                  child: tuteur["imageUrl"] == null
                      ? Text(
                          '${(tuteur["prenom"] as String)[0]}${(tuteur["nom"] as String)[0]}'.toUpperCase(),
                          style: TextStyle(
          ...
          Container(
            ...
            child: Row(
              mainAxisSize: MainAxisSize.min,
              children: [
                Icon(Icons.star_rounded, color: Colors.amber, size: rank == 1 ? 16 : 14),
    ...
    return Container(
      ...
      child: Column(
        children: _otherTutors.asMap().entries.map((entry) {
          ...
          return _buildOtherTutorListItem(tuteur, rank, isDark, textColor, index == _otherTutors.length - 1);
        }).toList(),
      ),
    );
  }

  Widget _buildOtherTutorListItem(Map<String, dynamic> tuteur, int rank, bool isDark, Color textColor, bool isLast) {
    return Column(
      children: [
        ListTile(
            ...
            child: Row(
              children: [
                ...
                Container(
                  ...
                  child: CircleAvatar(
                    backgroundColor: isDark ? Colors.grey.shade800 : EnsiConnectApp.ensisaLightBlue,
                    backgroundImage: tuteur["imageUrl"] != null
                        ? NetworkImage(tuteur["imageUrl"])
                        : null,
                    child: tuteur["imageUrl"] == null
                        ? Text(
                            '${(tuteur["prenom"] as String)[0]}${(tuteur["nom"] as String)[0]}'
                                .toUpperCase(),
                            style: TextStyle(
                              color: isDark ? Colors.white : EnsiConnectApp.ensisaBlue,
                              fontWeight: FontWeight.bold,
