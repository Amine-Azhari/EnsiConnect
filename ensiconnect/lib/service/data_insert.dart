...
class FirebaseDataService {
  ...
  Future<void> initialiserDonneesDeTest() async {
    ...
    try {
      ...
      DocumentReference refMaths =
          await _db.collection('Matiere').add({'Nom': 'Algèbre Linéaire'});
      DocumentReference refInfo = await _db
          .collection('Matiere')
          .add({'Nom': 'Programmation Orientée Objet'});
      DocumentReference refPhysique =
          await _db.collection('Matiere').add({'Nom': 'Thermodynamique'});

      // Insertion des Salles
      DocumentReference refSalleA =
          await _db.collection('Salle').add({'Nom': 'Salle TD 101'});
      ...
      await _db.collection('RejoindreSession').add({
        'EtudiantID': refAlice.id,
        'SessionID': refSessionMaths.id,
        'Date': '2026-06-05',
        'Contenu': 'Super !'
      });
      ...
      await _db.collection('Evaluation').add({
        'EvaluateurID': refAlice.id,
        'EvalueID': refLucas.id,
        'SessionID': refSessionMaths.id,
        'Note': 5
      });
      ...
      await _db.collection('Messagerie').add({
        'ExpediteurID': refLucas.id,
        'DestinataireID': refSarah.id,
        'SessionID': null,
        'Date': '2026-06-06 18:30',
        'Contenu': 'Salut Sarah, on peut se voir demain !'
      });
